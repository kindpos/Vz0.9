...
export default function LoginScreen({ onLogin, roster = [] }) {
  ...
  const handleClockIn = async () => {
    if (!pin) { setError("Enter PIN first."); return; }
    const staff = roster.find(r => r.pin === pin);
    if (!staff) { setError("PIN not recognised."); setPin(""); return; }
    try {
      const res = await fetch(`${API_BASE}/api/v1/servers/clock-in`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ employee_id: staff.id, employee_name: staff.name, pin }),
      });
      const data = await res.json();
      if (data.success) {
        setClockMsg(`${staff.name} clocked IN at ${new Date(data.clocked_in_at).toLocaleTimeString()}`);
      } else {
  ...
  const handleClockOut = async () => {
    ...
    if (!staff) { setError("PIN not recognised."); setPin(""); return; }
    try {
      const res = await fetch(`${API_BASE}/api/v1/servers/clock-out`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ employee_id: staff.id, employee_name: staff.name }),
      });
      const data = await res.json();
      if (data.success) {
        setClockMsg(`${staff.name} clocked OUT at ${new Date(data.clocked_out_at).toLocaleTimeString()}`);
      } else {
    ...
    <div style={T.body}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
        <div style={{ width: 172 }}>
          ...
          {clockMsg && (
            <div style={{ background: "#ccffcc", border: `1px solid #006400`, padding: "2px 6px", fontSize: 10, marginTop: 4, color: C.green }}>
              {clockMsg}
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={handleClockIn} style={{ ...T.btn, minWidth: 0, padding: "4px 12px", fontSize: 10 }}>Clock In</button>
          <button onClick={handleClockOut} style={{ ...T.btn, minWidth: 0, padding: "4px 12px", fontSize: 10 }}>Clock Out</button>
        </div>
      </div>
    </div>
  );
